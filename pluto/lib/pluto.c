...
unsigned cut_final_solution_sccs(plutoCost_Matrix *pluto_trans_Cost, Graph *ddg,
                                 int *topSortList) {
  ...
  IF_DEBUG(printf("[pluto] cut_final_solution_sccs\n"));

  if (ddg->num_sccs == 1) {
    IF_DEBUG(printf("\tonly one SCC\n"));
    return 0;
  }
  int cutList[ddg->num_sccs];
  ...
  for (unsigned i = 0; i < nstmts; i++) {
    ...
    stmts[i]->trans->val[stmts[i]->trans->nrows - 1][nvar + npar] =
        pluto_trans_Cost->fusionCondition[cutList[stmts[i]->scc_id]];
  }
...
//Partition of SCC
unsigned cut_m_n_sccs(plutoCost_Matrix *pluto_trans_Cost, Graph *ddg,
                      int *topSortList) {
  PlutoProg *prog = pluto_trans_Cost->prog;
  Stmt **stmts = prog->stmts;
  int nstmts = prog->nstmts;
  ...
  IF_DEBUG(printf("[pluto] cut_m_n_sccs\n"));
  ...
  pluto_prog_add_hyperplane(prog, prog->num_hyperplanes, H_SCALAR);
  int transList[ddg->num_sccs];
  int cutList[ddg->num_sccs];
  for (int i = 0; i < ddg->num_sccs; i++) {
    transList[topSortList[i]] = i;
    cutList[topSortList[i]] = 0;
  }
  IF_DEBUG(fprintf(stdout, "\nstart_scc_id:%d ,end_scc_id:%d \n",
                   pluto_trans_Cost->start_scc_id,
                   pluto_trans_Cost->end_scc_id));
  // Start dividing the scc of m-n
  int cutCount = 0;
  for (int index = 0; index < ddg->num_sccs; index++) {
    for (int i = 0; i < ddg->num_sccs; i++) {
      if (transList[i] == index) {
        if (transList[i] < pluto_trans_Cost->start_scc_id) {
          IF_DEBUG(fprintf(stdout, "\n < start_scc_id cutCount:%d", cutCount););
          cutList[i] = cutCount;
          cutCount++;
        } else if (transList[i] > pluto_trans_Cost->end_scc_id) {
          cutCount++;
          IF_DEBUG(fprintf(stdout, "\n > end_scc_id cutCount:%d", cutCount););
          cutList[i] = cutCount;
        } else if (transList[i] <= pluto_trans_Cost->end_scc_id &&
                   transList[i] >= pluto_trans_Cost->start_scc_id) {
          cutList[i] = cutCount;
          IF_DEBUG(fprintf(stdout, "\n inner cutCount:%d", cutCount););
        }
      }
    }
  }
  IF_DEBUG(
      fprintf(stdout, "\n transList \n");
      for (int i = 0; i < ddg->num_sccs; i++) {
        fprintf(stdout, " %d ", transList[i]);
      } fprintf(stdout, "\n cutList \n");
      for (int i = 0; i < ddg->num_sccs; i++) {
        fprintf(stdout, " %d ", cutList[i]);
      } fprintf(stdout, "\n \n"););
...
/*
0-1 constraints
1 * c_1 + 1* c_2 +1 *c_3....+1 * c_n + -1 = 0
*/
PlutoConstraints *get_zore_one_sol_constraints(const PlutoProg *prog,
  ...
  PlutoConstraints *onecst = pluto_constraints_alloc(nstmts, ncols, context);
  for (int i = 0; i < nstmts; i++) {
    onecst->is_eq[i] = 1;
  }
  onecst->ncols = ncols;
  ...
  for (int i = 0; i < nstmts; i++) {
    ...
    for (int j = 0; j < (int)prog->stmts[i]->dim; j++) {
      onecst->val[onecst->nrows][stmt_offset + j] = 1;
    }

    onecst->val[onecst->nrows][ncols - 1] = -1;
    onecst->nrows++;
  }
...
When num_sols_found <= scc.dim-1 looks for a hyperplane, add interchange constraints
2 * c_1 + 1* c_2 +1 *c_3 + -1 = 0
*/
PlutoConstraints *
get_interchange_sol_constraints(const PlutoProg *prog,
                                plutoCost_Matrix *pluto_trans_Cost,
                                int *topSortList, bool hyp_search_mode) {
  Stmt **stmts = prog->stmts;
  int nstmts = prog->nstmts;
  int nvar = prog->nvar;
  int npar = prog->npar;
  ...
  // Get m,n, k: Move the KTH variable of the [m,n] statement to the innermost layer
  int start_pos = pluto_trans_Cost->start_scc_id; // m
  int end_pos = pluto_trans_Cost->end_scc_id;     // n
  int num_iter = pluto_trans_Cost->iter;          // k
  int stmt_num = 0;
  int flag_stmt[nstmts];
  for (int i = 0; i < nstmts; i++) {
    flag_stmt[i] = 0; 
  }
  for (int j = start_pos; j <= end_pos; j++) {
    for (int i = 0; i < nstmts; i++) {
      if (prog->stmts[i]->scc_id == topSortList[j]) {
        stmt_num++;
        // stmt_dim = scc_dim[topSortList[j]];
        flag_stmt[i] = 1;
      }
    }
  }
  PlutoConstraints *interchangecst =
      pluto_constraints_alloc(stmt_num, ncols, context);
  // 申请stmt_num个等式，is_eq代表申请的是等式
  for (int i = 0; i < stmt_num; i++) {
    interchangecst->is_eq[i] = 1;
  }
  interchangecst->ncols = ncols;
  int coeff_offset = npar + 1 + ncols - CST_WIDTH;
  for (int i = 0; i < nstmts; i++) {
    /* Don't add the constraint if enough solutions have been found */
    if (pluto_stmt_get_num_ind_hyps(stmts[i]) >= stmts[i]->dim_orig) {
    ...
    if (flag_stmt[i] == 1) {             
      int stmt_offset =
          coeff_offset + i * (nvar + 1); 
      for (int j = 0; j < (int)prog->stmts[i]->dim;
           j++) { 
        if (j == num_iter)
          interchangecst->val[interchangecst->nrows][stmt_offset + j] =
              2; 
        else
          interchangecst->val[interchangecst->nrows][stmt_offset + j] =
              1; 
      }
      interchangecst->val[interchangecst->nrows][ncols - 1] =
          -1; 
      interchangecst->nrows++;
    }
  }
  return interchangecst;
}

PlutoConstraints *get_final_interchange_sol_constraints(
    const PlutoProg *prog, plutoCost_Matrix *pluto_trans_Cost, int *topSortList,
    bool hyp_search_mode, int *scc_dim, int num_sols_found) {
  Stmt **stmts = prog->stmts;
  int nstmts = prog->nstmts;
  // Maximum depth
  int nvar = prog->nvar;
  ...
  //If SCC dim already = nvar, there is no need to add it
  int cutList[ddg->num_sccs];
  for (int i = 0; i < ddg->num_sccs; i++) {
    for (int j = 0; j < ddg->num_sccs; j++) {
      if (i == topSortList[j]) {
  ...
  PlutoConstraints *interchangecst =
      pluto_constraints_alloc(nstmts, ncols, context);
  // stmt_num equality is applied for. is_eq indicates that equality is applied for
  for (int i = 0; i < nstmts; i++) {
    interchangecst->is_eq[i] = 1;
  }
  ...
  for (int i = 0; i < nstmts; i++) {
    /* Don't add the constraint if enough solutions have been found */
    // If the hyperplane is already full there is no need to add a constraint
    if (pluto_stmt_get_num_ind_hyps(stmts[i]) >= stmts[i]->dim_orig) {
      IF_DEBUG2(fprintf(stdout, "non-zero cst: skipping stmt %d\n", i));
      continue;
    }
    ...
    int stmt_offset = coeff_offset + i * (nvar + 1);
    for (int j = 0; j < (int)prog->stmts[i]->dim; j++) {
      if (j == pluto_trans_Cost->interchCondition[cutList[stmts[i]->scc_id]] &&
          num_sols_found < scc_dim[stmts[i]->scc_id] - 1)
        interchangecst->val[interchangecst->nrows][stmt_offset + j] =
            2;
      else
        interchangecst->val[interchangecst->nrows][stmt_offset + j] =
...
int find_our_hyperplanes(plutoCost_Matrix *pluto_trans_Cost,
                         bool hyp_search_mode, int max_sols, int band_depth,
                         int *topSortList, int *scc_dim) {
  PlutoProg *prog = pluto_trans_Cost->prog;
  int nstmts = prog->nstmts;
  int nvar = prog->nvar;
  int npar = prog->npar;
  ...
  if (max_sols == 0)
    return 0;
  IF_DEBUG(fprintf(
      stdout, "\n-----------------enter find_our_hyperplanes--------------\n"));
  /* Don't free basecst */
  // Add base and boundary constraints
  ...
  do {
    ...
    /*hyp_search_mode
      =EAGER:If the transformation of the statement is not full-ranked, then the hyperplane found is the cyclic hyperplane.
      = LAZY:For non-holonomic statements, at least one hyperplane should be a cyclic hyperplane
    */
    ...
    // Add 0-1 constraint equality constraints
    PlutoConstraints *onecst =
        get_zore_one_sol_constraints(prog, hyp_search_mode);
    pluto_constraints_add(currcst, onecst);
    pluto_constraints_free(onecst);

    int stmt_dim = 0;
    for (int i = 0; i < nstmts; i++) {
      if (prog->stmts[i]->scc_id ==
          topSortList[pluto_trans_Cost->start_scc_id]) {
        stmt_dim = scc_dim[topSortList[pluto_trans_Cost->start_scc_id]];
      }
    }
   
    if (num_sols_found < prog->nvar - 1 && pluto_trans_Cost->is_global) {
      PlutoConstraints *final_interchange =
          get_final_interchange_sol_constraints(prog, pluto_trans_Cost,
                                                topSortList, hyp_search_mode,
                                                scc_dim, num_sols_found);
      pluto_constraints_add(currcst, final_interchange);
      IF_DEBUG(
          fprintf(
              stdout,
              "\n-----------------printf global interchangecst-------------\n");
          pluto_constraints_print(stdout, final_interchange););
      pluto_constraints_free(final_interchange);
    } else if (num_sols_found < stmt_dim - 1 && !pluto_trans_Cost->is_global) {
      PlutoConstraints *interchangecst = get_interchange_sol_constraints(
          prog, pluto_trans_Cost, topSortList, hyp_search_mode);
      pluto_constraints_add(currcst, interchangecst);
      IF_DEBUG(
          fprintf(stdout,
                  "\n-----------------printf interchangecst-------------\n");
          pluto_constraints_print(stdout, interchangecst););
      pluto_constraints_free(interchangecst);
    }
    ...
    if (bestsol != NULL) {
      ...
      IF_DEBUG(fprintf(stdout, "[pluto] find_permutable_hyperplanes:add "
                               "interchange cst No hyperplane found\n"));
      // If the addition constraint does not find the hyperplane, then simply jump out and use the original solution with Cost = -1
...
/// start, the hyperplane that will form the cone with the conc start
/// hyperplane cone_complement_hyps will set to the cone complement
/// hyperplanes found for statements in the band.
static int
find_cone_complement_hyperplane(Band *band, PlutoMatrix *conc_start_faces,
  ...
  // TODO: improve comment by including info on the constraints that are
  // added.
...
// prog_free
void my_pluto_prog_free(PlutoProg *prog) {
  /* Free dependences */
...
int get_scc_num(PlutoProg *prog) { return prog->ddg->num_sccs; }
int get_stmt_num(PlutoProg *prog) {
  int stmt_num = 0;
  stmt_num = PLMAX(stmt_num, (int)prog->nstmts);
  return stmt_num;
}
//Check whether the topology sort has the largest number of SCC neighbors of the same layer
bool judge_topSort(int *topSortList, int *tempSortList, int n, PlutoProg *prog,
                   int *scc_dim) {
  PlutoContext *context = prog->context;
  int top_num = 0;
  int temp_num = 0;
  for (int i = 1; i < n; i++) {
    if (scc_dim[topSortList[i]] == scc_dim[topSortList[i - 1]]) {
      top_num++;
    }
    if (scc_dim[tempSortList[i]] == scc_dim[tempSortList[i - 1]]) {
      temp_num++;
    }
  }
  if (temp_num > top_num) {
...
bool topSortList_empty(int topSortList[], int n) {
  for (int i = 0; i < n; i++) {
    if (topSortList[i] != 0) {
      return false;
    }
...
// Get the depth of each scc sorted by scc_id before sorting
void get_scc_dim(PlutoProg *prog, int n, int *scc_dim) {
  PlutoContext *context = prog->context;
  int nstmts = prog->nstmts;
  // Sort by scc_id
  for (int j = 0; j < prog->ddg->num_sccs; j++) {
    ...
    for (int i = 0; i < prog->nstmts; i++) {
      if (prog->stmts[i]->scc_id == j) {
        test = PLMAX(test, (int)prog->stmts[i]->dim);
        scc_dim[j] = test;
      }
    }
  }
  ...
  IF_DEBUG(
      ...
      fprintf(stdout, "\n-----output stmt_scc_id and stmt dim-----\n");
      for (int i = 0; i < nstmts; i++) {
        fprintf(stdout, "\nstmt[%d].scc_id %d , dim: %d\n", i,
                prog->stmts[i]->scc_id, prog->stmts[i]->dim);
      }

  );
}
// Search topological sort
int dfs_topSort(int num, int n, int indegree[], int visit[], int topSortList[],
                int tempSortList[], int **scc_id_adj, PlutoProg *prog,
                int scc_dim[]) {
  PlutoContext *context = prog->context;
  if (num == n) { 
    if (topSortList_empty(topSortList, n)) {
      for (int i = 0; i < n; i++) {
        topSortList[i] = tempSortList[i];
      }
    } else {
      // Check whether the topology sort has the largest number of SCC neighbors of the same layer
      if (judge_topSort(topSortList, tempSortList, n, prog, scc_dim)) {
        for (int i = 0; i < n; i++) {
          topSortList[i] = tempSortList[i];
        }
      }
  ...
  for (int i = 0; i < n; i++) {
    if ((!indegree[i]) && (!visit[i])) { 
      for (int j = 0; j < n; j++) {
        if (scc_id_adj[i][j] != 0) {
          indegree[j] -= scc_id_adj[i][j];
        }
      }
      // fprintf(stdout, "\nmeet the condition:%d\n", i);
      visit[i] = 1;        
      tempSortList[num] = i; 
      dfs_topSort(num + 1, n, indegree, visit, topSortList, tempSortList,
                  scc_id_adj, prog, scc_dim); 
      for (int j = 0; j < n; j++) {
        if (scc_id_adj[i][j]) {
          indegree[j] += scc_id_adj[i][j];
        }
...
// Sort by scc_id topology
void find_topSort(PlutoProg *prog, int *topSortList, int *scc_dim) {
  Stmt **stmts = prog->stmts;
  int nstmts = prog->nstmts;
  PlutoContext *context = prog->context;
  int **scc_id_adj;
  get_scc_dim(prog, prog->ddg->num_sccs, scc_dim);
  scc_id_adj = (int **)malloc(prog->ddg->num_sccs * sizeof(int *));
  for (int i = 0; i < prog->ddg->num_sccs; i++) {
    scc_id_adj[i] = (int *)malloc(prog->ddg->num_sccs * sizeof(int));
  }
  ...
  //The degree of each vertex (SCC_id)
  int indegree[prog->ddg->num_sccs];
  int visit[prog->ddg->num_sccs];
  int tempSortList[prog->ddg->num_sccs]; // The results of each topology sort are temporarily stored
  for (int i = 0; i < prog->ddg->num_sccs; i++) {
    tempSortList[i] = 0;
  }
  for (int i = 0; i < prog->ddg->num_sccs; i++) {
  ...
  for (int i = 0; i < prog->ndeps; i++) {
    Dep *dep = prog->deps[i];
    // Keep only RAW dependencies
    if (dep->type == PLUTO_DEP_RAW) {
      int front = prog->stmts[dep->src]->scc_id;
      int back = prog->stmts[dep->dest]->scc_id;
      //Is RAW and does not point itself to a ring, here is to delete the excess self-edge
      if (front != back) {
        // Adjacency matrix of SCC_ID
        scc_id_adj[front][back]++;
        // Calculate the degree of each SCC_id
        indegree[back]++;
      }
    }
  }
  ...
  int sum_topSort = 1;
  sum_topSort =
      dfs_topSort(0, prog->ddg->num_sccs, indegree, visit, topSortList,
                  tempSortList, scc_id_adj, prog, scc_dim);
  IF_DEBUG(fprintf(stdout, "\n----- DFS_topSort Done---sum_topSort = %d----\n",
                   sum_topSort););
}

//If the depth of the scc from m to n is different, return false and set the Cost to -1
bool m_n_scc_dim_dif(PlutoProg *prog, int *topSortList, int *scc_dim, int start,
                     int end) {
  if (start == end) {
    return true;
  } else {
    for (int i = start; i <= end; i++) {
      for (int j = start; j <= end; j++)
        if (scc_dim[topSortList[i]] != scc_dim[topSortList[j]]) {
          return false;
        }
    }
  }
...
//our transformation
int pluto_our_transform(plutoCost_Matrix *pluto_trans_Cost, int *topSortList,
                        int *scc_dim) {
  PlutoProg *prog = pluto_trans_Cost->prog;
  ...
  options->fuse = 0;
  if (pluto_trans_Cost->is_global == 1) {
    cut_final_solution_sccs(pluto_trans_Cost, ddg, topSortList);
  } else {
    cut_m_n_sccs(pluto_trans_Cost, ddg, topSortList);
  }
  ...
  for (s = 0; s < nstmts; s++) {
    ...
    max_sol_num =
        PLMAX(max_sol_num, (int)stmts[s]->dim_orig -
  ...
  if (options->dfp) {
     ...
     * colouring has to start from first level. Internally this is also used
     * to introduce must distribute edges in the fusion conflict graph. */
    ...
    do {
      ...
      nsols = find_our_hyperplanes(pluto_trans_Cost, hyp_search_mode,
                                   num_sols_left, depth, topSortList, scc_dim);
      ...
      if (nsols == max_sol_num) {
        ...
        if (hyp_search_mode == EAGER) {
          ...
          //loop transformation failed under constraints
          pluto_trans_Cost->is_succeed = false;
          return 1;
        }
      }
...
int pluto_Max_No_transform(plutoCost_Matrix *pluto_trans_Cost) {
  int i, j, s, nsols, depth;
  /* The maximum number of linearly independent solutions needed across all
   * statements */
  int num_ind_sols_req;
  ...
  // 开始写对照组
  if (pluto_trans_Cost->is_global == 2) {
    options->fuse = kMaximalFuse;
  } else if (pluto_trans_Cost->is_global == 3) {
  ...
  if (pluto_trans_Cost->is_global == 4) {
    // if (options->debug || options->moredebug) {
    //   printf("\tNumber of unsatisfied deps: %d\n",
    //          get_num_unsatisfied_deps(prog->deps, prog->ndeps));
    //   printf("\tNumber of unsatisfied inter-scc deps: %d\n",
...
/// Returns true if these statements completely fused until the innermost
/// level. This assumes one-to-one transformations for statements.
