...
void Register::getLURSet(int time,vector<int> &candidataR)
{
	...
	for (auto node : this->TERnodesList)
	{
		if (node->RegisterKind == 1 && node->time == time && node->isOccupied == false)/* 结果寄存器 */
		{
			candidataR.push_back(node->RegisterID);
		}
...
{   
	...
	for(int i = 0; i < C->CGRAnodesList.size(); i++)
	{
		...
		if(C->CGRAnodesList[i]->ElmKind == 1)/* LU */
		{
			...
			for(int m = 0; m <  C->CGRAnodesList[i]->ElmNeighbors.size(); m++)
			{
				...
				int neiR = calculateR(neiPE);
				for(int m = 0; m < 4; m++)
				{
					node->RegisterNeighbors.push_back(neiR + 2 + m);
				}
			...
			for(int j = 0; j < Rnum ; j++)
			{
				...
				for(int m = 0; m < neighNum; m++)
				{
					...
					if(C->CGRAnodesList[neighbor]->ElmKind == 0)/* 如果邻居是PE */
					{
						/* 该邻居的前四个寄存器是 邻居 */
						/* 该PE邻居 */
						int base = calculateR(neighbor);
						
						for(int n = 1; n < Rnum; n++)
						{
							node->RegisterNeighbors.push_back(base + 1 + n);
						}
				
					}
					else if(C->CGRAnodesList[neighbor]->ElmKind == 2 || C->CGRAnodesList[neighbor]->ElmKind == 1 )/* 如果邻居是su*/
					{
						int su = neighbor/ 6 *26 + neighbor % 6 ;
						node->RegisterNeighbors.push_back(su);/*  */
					}
...
void Register::CreatTER(int II)
{
	...
	for(int i = 0; i < II; i++)/* 遍历所有的寄存器节点，平面的 */
	{
		for(int j = 0; j < this->RnodesNums; j++)/* 遍历所有的平面寄存器 */
		{
			...
			if(this->RnodesList[j]->RegisterKind == 1)/* LU */
			{
