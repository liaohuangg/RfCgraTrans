...
LogicalResult Importer::processStmt(clast_assignment *ass) {
  ...
  mlir::Operation *op;
  if (substMap.isSingleConstant()) {//如果loop bound是常数
    op = b.create<mlir::ConstantOp>(
        b.getUnknownLoc(), b.getIndexType(),
        b.getIntegerAttr(b.getIndexType(), substMap.getSingleConstantResult()));
  } else if (substMap.getNumResults() == 1) {//如果loop bound是只有一个返回值，也就是说单纯的偏移
    op = b.create<mlir::AffineApplyOp>(b.getUnknownLoc(), substMap,
                                       substOperands);
  } else {//如果loop bound有多个返回值，有max和min这种情况
...
mlir::Operation *RfCgraTrans::judgeFuncOpFromOpenScop(
    std::unique_ptr<OslScop> scop, ModuleOp module, OslSymbolTable &symTable,
    MLIRContext *context, PlutoProg *prog, const char *dumpClastAfterPluto,int times) {
  ...
  FILE *clastPrintFile = stderr;
  if (dumpClastAfterPluto) {
    int len  = strlen(dumpClastAfterPluto);
    std::string filename = dumpClastAfterPluto;
    std::string time_str = std::to_string(times);
    //复制之后再插入次数
    filename.insert(len-6,time_str);
    // os<<"filename = "<<filename<<" \n";
    char filename_c[40];
    std::strcpy(filename_c,filename.c_str());
    clastPrintFile = fopen(filename_c, "w");
    assert(clastPrintFile &&
           "File for clast dump after Pluto cannot be opened.");
  }
...
mlir::Operation *RfCgraTrans::finalFuncOpFromOpenScop(
    std::unique_ptr<OslScop> scop, ModuleOp module, OslSymbolTable &symTable,
    MLIRContext *context, PlutoProg *prog, const char *dumpClastAfterPluto,int times) {
  // TODO: turn these C struct into C++ classes.
  CloogState *state = cloog_state_malloc();
...
mlir::Operation *RfCgraTrans::createFuncOpFromOpenScop(
    std::unique_ptr<OslScop> scop, ModuleOp module, OslSymbolTable &symTable,
...
OwningModuleRef
RfCgraTrans::translateOpenScopToModule(std::unique_ptr<OslScop> scop,
                                   MLIRContext *context) {
