...
int calculateR(int neiPE)
{
    int pecolum = 6;
    return ((neiPE % pecolum) - 2 + neiPE / pecolum *4) * 6 + (neiPE / pecolum + 1) *2;
}
...
//     fos << "********************************" << endl;
//     fos << "the name of File:"  << FLAGS_dfg_file << endl;
//     fos << "********************************" << endl;
//     string s;
//     int nodes = 0;
//     vector<int> linedata;
...
bool compLegal(Register *R,vector<int> sameSrcEdge, vector<int> tempTruePath)
{
    ...
    // fos<<"sameSrcEdge.size()="<<length1<<"  "<<"tempTruePath.size()="<<length2<<endl;
    if(length1 > length2)
    {
        longlength = length1;
        shortlength = length2;
    ...
    int cgraPE =24;
    int label =0;
    // fos<<"tempTruePath[length2-1]="<<tempTruePath[length2-1]<<endl;
    //  fos<<"shortlength="<<shortlength<<endl;
    for(int i = 0; i < shortlength ; i++ )
    {
        if(sameSrcEdge[i] != tempTruePath[i])
        {
            ...
            label = i;
            // fos<<"i="<<i<<endl;
            // fos<<"sameSrcEdge[i]="<<sameSrcEdge[i]<<" "<<"tempTruePath[i]="<<tempTruePath[i]<<endl;
    ...
    // fos<<"label"<<label<<endl;
    // fos<<"length1-2="<<length1-2<<endl;
    // fos<<"length2="<<length2<<endl
    if(label != 0)
    {
            ...
            for(int i = label; i < length1-2 ; i++ )/* 已经布好的 */
            {
                for(int j = label; j < length2 ; j++ )
                {
                    if(sameSrcEdge[i] == tempTruePath[j])
                    {
                        ...
                        if(i-1 >=0)
                        {
                            // fos<<"sameSrcEdge[i]="<<sameSrcEdge[i]<<" "<<"tempTruePath[j]="<<tempTruePath[j]<<endl;
                            // fos<<"sameSrcEdge[i]="<<sameSrcEdge[i-1]<<" "<<"tempTruePath[j]="<<tempTruePath[j]<<endl;
                            /* 平面的PE不同 */
                            if((R->TERnodesList[sameSrcEdge[i-1]]->PE % cgraPE != R->TERnodesList[sameSrcEdge[i]]->PE%cgraPE) && (R->TERnodesList[tempTruePath[j-1]]->PE%cgraPE != R->TERnodesList[tempTruePath[j]]->PE%cgraPE ))
                            {
                                ...
                                // fos<<"R->TERnodesList[sameSrcEdge[i-1]]->PE="<<R->TERnodesList[sameSrcEdge[i-1]]->PE<<"  "<<" R->TERnodesList[tempTruePath[j-1]]->PE2="<< R->TERnodesList[sameSrcEdge[i]]->PE<<endl;
                                return false;
                            }
...
bool isLegalShare(Register *R, int i,int *path,int srcTrueTime,vector<vector<int>> sameSrcEdge,vector<int> tempTruePath,int *inportNum,int *outportNum)
{
    ...
    // fos<<"tempTruePath,.size()="<<tempTruePath.size()<<"  "<<"truei="<<truei<<endl;
    // fos<<endl;
    ...
    /* 遍历前面已经布线的路径 */
    bool isLegal = true;
    // fos<<"sameSrcEdge.size()="<<sameSrcEdge.size()<<endl;
    for(int i = 0; i < sameSrcEdge.size(); i++ )
    {
...
// void dijkstra(Register *R,GraphRegister *GR,int latency,int virtualSrc,vector<int> &virtualCandidate,vector<int> &pathNode,int trueTRNum,int srcTrueTime,vector<vector<int>> sameSrcEdge,vector<int> SingleTruePath)
{
    ...
    // fos<<"nodeNum"<<nodeNum<<endl;
    // fos<<"virtualCandidate.size()="<<virtualCandidate.size()<<endl;
    // fos<<"nodeNum"<<nodeNum<<endl;
    ...
    for (int i = 0; i < GR->GraphedgesList.size(); i++)
    {
        ...
        // {
        //     fos<<"GR->GraphedgesList[i]->value"<<GR->GraphedgesList[i]->value<<endl;
        // }
    ...
    if(dijk->latency == 9)
    {
        ...
        // fos<<"dijk->virtualSrc="<<dijk->virtualSrc<<endl;
        // fos<<"Graph[31][147]="<<Graph[2][13]<<endl;
        // fos<<"Graph[31][147]="<<Graph[2][14]<<endl;
        // fos<<"Graph[31][147]="<<Graph[2][15]<<endl;
        // // fos<<"Graph[31][147]="<<Graph[348][455]<<endl;

        // fos<<"Graph[31][147]="<<Graph[230][343]<<endl;
        // fos<<"Graph[31][147]="<<Graph[343][455]<<endl;
    ...
    /* 初始化 */
    // fos<<"dijk->latency="<<dijk->latency<<endl;
    // fos<<"dijk->virtualSrc="<<dijk->virtualSrc<<endl;
    for(int i = 0; i < nodeNum; ++i)
    {
    ...
    // fos<<"dijk->virtualSrc="<<dijk->virtualSrc<<"  "<<"dijk->virtualSrc="<<dijk->virtualSrc<<endl;
    ...
    // fos<<"virtualCandidate="<<virtualCandidate[0]<<endl;
...
srcR:该前驱绑定的资源 */
int shareRoute(DFG *D, Register *R, int prenodeIndex, int srcR, int II, AllPath *allPathClass,int *inportNum,int *outportNum)
{
    std::ofstream fos;
    fos.open("MapInformation.out", std::ios::app);
    vector<DFGedge*>  DFGedgeFromLoad = getEdgesFromBegin(D->DFGedgesList,prenodeIndex);/* 获得以该Node为源头的边 */
    // fos<<"DFGedgeFromLoad.size()"<<DFGedgeFromLoad.size()<<endl;
    ...
    {
        // fos<<"prenode="<<DFGedgeFromLoad[i]->prenode<<"      posenode="<<DFGedgeFromLoad[i]->posnode<<"       latency"<<DFGedgeFromLoad[i]->latency<<endl;
        ...
        if(D->DFGnodesList[posnodeIndex]->isBind == false)/* 没有被绑定 */
        {
            if(desKind == 2)/* SU */
            {
                ...
                // fos<<"candidataR.size()="<<candidataR.size()<<endl;
               
            }
            else if(desKind == 0)/* 否则就是普通的计算算子 */ /* 需要放在真实结果寄存器中 */
            {
                // fos<<"desTime="<<desTime<<endl;
                R->getResultRSet(desTime,candidataR);
                // fos<<"candidataR.size()="<<candidataR.size()<<endl;
                // fos<<"candidataR[0]="<<candidataR[0]<<endl;
            }
        ...
        // fos<<"virtualSrc="<<virtualSrc<<endl;
        ...
        for(int m = 0; m < candidataR.size(); m++)
        {
            ...
            // fos<<"candidatashareRouteR[m]="<<candidataR[m]<<endl;
        }
        ...
        // fos<<"restLatency="<<restLatency<<endl;
        ...
        // fos<<"dijSuccess="<<dijSuccess<<endl;
        if(dijSuccess == false)
        {
            fos<<"#################################"<<endl;
            fos<<"dijkstra short can't find path"<<endl;
            fos<<"#################################"<<endl;
            return 0;
        }
        ...
        // fos<<"lastnodeInPath="<<lastnodeInPath<<endl;
        // fos<<"posnodeIndex="<<posnodeIndex<<endl;
        if(desKind == 0)
        {
        ...
        // fos<<"posnodeIndex="<<posnodeIndex<<"  "<<"D->DFGnodesList[posnodeIndex]->bindResource ="<<D->DFGnodesList[posnodeIndex]->bindResource <<endl;
    ...
    for(int i = 0; i < sameSrcEdge.size(); i++)/* 同源的所有长依赖 */
    {
        for(int j = 0; j < sameSrcEdge[i].size() - 1; j++)/*  */
        {
            ...
            {
                // fos<<"node1=" <<node1 <<"  " <<"node2=" <<node2 <<endl;
