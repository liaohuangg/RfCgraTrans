...
/// 'Block' class.
// alignas(8) 作为类型说明符之一出现在声明语法中，以修改所声明对象的对齐要求。
// final 
//1.不希望类被继承的时候 例如:这里的使用   
//2.不希望父类的某个地方被子类覆盖/重写时  例如:访问修饰符 final 返回类型 方法名
//3.当不希望类的某个属性的值被修改，可以用final修饰  例如:public final double TAX_RATE = 0.08
//4.当不希望某个局部变量被修改，可以使用final修饰 例如:final double NUM= 0.01
