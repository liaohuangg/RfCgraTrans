...
DFG::DFG(int II, int childNum,int numDFGnodes,vector<vector<int>> DFG_node,int dfg_id,int solutionNum)
{
    int i, j, k;
    ...
    // numDFGnodes = lines;
    std::ofstream fos;
    fos.open("map"+to_string(dfg_id)+"_solu"+to_string(solutionNum)+".txt", std::ios::app);
    this->numDFGnodes = numDFGnodes;
    int count_nodes = 0;
    ...
    for (int i = 0; i < DFG_node.size(); i++){
        ...
        int *child = (int *)malloc(sizeof(int)*childNum); 
        int *childDif = (int *)malloc(sizeof(int)*childNum); 
    ...
    for (int i = 0; i < numDFGedges; i++)
    {
       ...
       // fos<<DFGedgesList[i]->edgeorder<<" "<<DFGedgesList[i]->prenode<<" "<<DFGedgesList[i]->posnode<<" "<<DFGedgesList[i]->dif<<" "<<"\n";
    }
    //fos<<"the number of long latency:"<<longcount<<"\n";
...
int DFG::Constraint_Level(int II){
    ...
    int *LoadNum = (int *)malloc(sizeof(int)*II); 
    int *SUNum = (int *)malloc(sizeof(int)*II); 
    int *PENum = (int *)malloc(sizeof(int)*II); 
    for(int i =0;i<II;i++){
            LoadNum[i]=0; 
            SUNum[i]=0;
            PENum[i]=0;
    ...
    for(i = 0; i < II; i++)
    {
        if(PENum[i] > 36 || LoadNum[i] > 6  || SUNum[i] > 6 )
        {
            flag = i;
