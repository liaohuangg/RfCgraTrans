...
void OslScopBuilder::buildScopStmtMap(mlir::FuncOp f,
                                      ...
                                      OslScop::ScopStmtMap *scopStmtMap) const {
  //如果parentOp不是ModuleOp 就返回类型不兼容
  mlir::ModuleOp m = cast<mlir::ModuleOp>(f->getParentOp());
   llvm::raw_ostream &os = llvm::outs();
  //walk根据所提供的回调，通过为每个嵌套操作(包括此操作)、块或区域调用回调来遍历操作。
  f.walk([&](mlir::Operation *op) {
    if (mlir::CallOp caller = dyn_cast<mlir::CallOp>(op)) {//如果是个调用操作
...
RfCgraTrans::createOpenScopFromFuncOp(mlir::FuncOp f, OslSymbolTable &symTable) {
...
mlir::LogicalResult RfCgraTrans::translateModuleToOpenScop(
...
void RfCgraTrans::registerToOpenScopTranslation() {
