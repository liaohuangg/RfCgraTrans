...
struct plutoCost_Matrix {
  ...
  //最后的整体变换
  // 0 不是全局 1 是全局 2 maxfuse 3 nofuse 4 原始
  int is_global;
...
typedef struct plutoCost_Matrix plutoCost_Matrix;

// struct minCost_Matrix {
//   //用来存放评估之后
//   int 
// };
// typedef struct minCost_Matrix minCost_Matrix;
/*
...
//
int *get_top_scc_dim(PlutoProg *prog, int topSortList[], int n,int scc_max_dim[]);
void my_pluto_prog_free(PlutoProg *prog);
int get_scc_num(PlutoProg *prog);
void get_scc_dim(PlutoProg *prog, int n, int *scc_dim);
int get_stmt_num(PlutoProg *prog);
void get_scc_dim_after_top(PlutoProg *prog, int *topSortList,int *scc_max_dim);
...
bool m_n_scc_dim_dif( PlutoProg *prog, int *topSortList,int *scc_dim,int start,int end);
int pluto_our_transform(plutoCost_Matrix *pluto_trans_Cost,int * topSortList,int *scc_dim);
//
...
int find_permutable_hyperplanes(PlutoProg *prog, bool lin_ind_mode,
 ...
 //
int find_our_hyperplanes(plutoCost_Matrix *pluto_trans_Cost,bool hyp_search_mode, int max_sols,
                         int band_depth,int * topSortList,int *scc_dim);
//
